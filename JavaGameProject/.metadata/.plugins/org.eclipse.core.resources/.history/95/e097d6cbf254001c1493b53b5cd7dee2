...
public class Server {
	...
	*/
	/*
	static Iterator get_iter(Selector sel) {
			...
			while (true) {
				...
				while (iterator.hasNext()) {
					...
					if (key.isAcceptable()) {
						...
						System.out.println("이까진 그냥 서버연결된거");
					...
					// 읽기 이벤트(클라이언트 -> 서버)가 발생한 경우
						...
						/* 어차피 set에 다 들어가있어서 굳이필요없음
						// 현재 아이디가 없을 경우 아이디 등록
						if (info.isID()) {	//이거는 한번만등록하면 넘어가기때문에 bytebuffer 써도 될듯?
						...
						*/
						...
						for(SocketChannel s : allClient) { 
							if (!readSocket.equals(s)) { //이게 자신을 제외한 사용자한테 보내는거
								...
								//s.write(outputBuf);	//사용자
								//outputBuf.flip();
								s.write(outputBuf3); //위치보내기
								
								//outputBuf3.flip();//이거 나중에 포함해서 테스트해봐
							}
						}
